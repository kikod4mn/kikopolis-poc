...
//        'php' => ['\<\?php', '\?\>']
	...
	public function output(bool $force_compile = false, string $return_type = 'cache'): string
	{
		...
		// Check for Corallize HTML tags.
		if (Config::ENABLE_CORALLIZE === true) {
			$coral  = new Corallize();
			$output = $coral->process($output);
		}
		// Check for Cardinal CMS commands.
		if (Config::ENABLE_CARDINAL) {
			$cardinal = new Cardinal();
			$output   = $cardinal->process($output);
		}
		// Check for Archon theme commands.
		if (Config::ENABLE_ARCHON) {
			$archon = new Archon();
			$output = $archon->process($output);
		}
//		var_dump($output);
